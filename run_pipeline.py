...
def field_is_usable(field: Any) -> bool:
    """
    Determine whether an extracted field contains
    a usable value.
    """
        ...
        for index, detection in enumerate(
            ...
            for neighbor in detections[
                ...
                repaired["mrp"] = {
                    ...
                    "note": (
                        "MRP label and numeric "
                        "value were detected "
                        "on adjacent OCR lines."
                    ),
                    "source_text": [
                        text,
                        neighbor_text,
    ...
    if (
        ...
        for index, detection in enumerate(
            ...
            if same_line:
                ...
                if (
                    value
                    not in invalid_manufacturer_values
                ):
            ...
            for neighbor in detections[
                ...
                repaired["manufacturer"] = {
                    ...
                    "note": (
                        "Manufacturer label and "
                        "company name were detected "
                        "on adjacent OCR lines."
                    ),
                    "source_text": [
                        text,
...
def extract_and_repair_fields(
    ...
    try:
        ...
        print(
            f"⚠️ Field extraction failed: {exc}"
        )
...
def determine_frontend_action(
    ...
    usable_fields = count_usable_fields(
        fields
    )
    ...
    # FIXED:
    # The original pasted code had a corrupted expression here.
    ...
    if (
        ...
        return frontend_result(
            action="ACCEPT",
            reason=(
                "All five required compliance "
                "fields were successfully "
                "extracted and passed validation."
            ),
            user_message=(
    ...
    if usable_fields == 0:
        ...
        return frontend_result(
            action="RETAKE",
            reason=(
                "No required compliance fields "
                "could be reliably extracted."
            ),
            user_message=(
                "Please retake the photo. Make "
                "sure the complete product label "
                "is visible, well lit, and in focus."
            ),
            instruction=(
    ...
    if (
        ...
        return frontend_result(
            action="RETAKE",
            reason=(
                "Required field(s) could not "
                f"be reliably extracted: {names}."
            ),
            user_message=(
                "Some required product information "
                "could not be read. Please retake "
                "the photo with the complete label "
                "clearly visible."
            ),
            instruction=(
                "Ask the user to capture "
    ...
    if (
        invalid_fields > 0
        or suspicious_fields > 0
    ):

        return frontend_result(
            action="REVIEW",
            reason=(
                "All required fields were detected, "
                "but "
                f"{invalid_fields} invalid and "
                f"{suspicious_fields} suspicious "
                "value(s) require verification."
            ),
            user_message=(
                "The product information was "
                "extracted, but some values need "
                "verification before a compliance "
    ...
    if (
        ...
        return frontend_result(
            ...
            reason=(
                ...
                "to safely pass the extracted "
                "information forward."
            ),
            user_message=(
                "The label text is difficult "
                "to read. Please retake the "
                "photo with better focus "
                "and lighting."
            ),
            instruction=(
                "Ask the user to capture "
    ...
    if compliance_decision in {
        ...
        return frontend_result(
            ...
            user_message=(
                "The product information was "
                "extracted, but manual verification "
                "is required."
            ),
            instruction=(
    ...
    return frontend_result(
        action="REVIEW",
        reason=(
            "The OCR result could not be safely "
            "classified as fully verified."
        ),
        user_message=(
            "The product information requires "
            "manual verification."
        ),
        instruction=(
...
def validate_image_path(
    ...
    if (
        ...
        raise ValueError(
            ...
            f"({size_mb:.1f} MB). Maximum allowed "
            f"is {MAX_IMAGE_FILE_SIZE_MB} MB."
        )
...
def run(
    ...
    print()
    print(
        "🔎 Stage 9: Checking image quality..."
    )
    ...
    print()
    print(
        "🔎 Attempt 1: Adaptive preprocessing..."
    )
    ...
    print(
        "📱 Stage 13: "
        "Determining frontend action..."
    )

    frontend_action = (
        determine_frontend_action(
            quality=quality,
            fields=fields,
    ...
    return {
        ...
        "label": frontend_action[
            "label"
        ],

        "next_step": frontend_action[
            "next_step"
        ],

        "user_message": frontend_action[
            "user_message"
        ],
    ...
    try:
        ...
        result = {
            ...
            "user_message": (
                ...
                "read. Please check the "
                "image path and try again."
            ),
